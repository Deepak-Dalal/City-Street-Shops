...
export default function SellerScreen() {
  ...
  const name = sellerName || "search seller products...";

  const [category, setCategory] = useState(productCategory || "Any");
  const [min, setMin] = useState(minPrice || 0);
  const [max, setMax] = useState(maxPrice || 0);
  const [rating, setRating] = useState(productRating || 0);
  const [order, setOrder] = useState(productsOrder || "newest");
  ...
  function filterSearchHandler() {
    ...
    navigate(
      `/seller/${sellerId}/name/${name}/category/${category}/min/${min}/max/${max}/rating/${rating}/order/${order}/pageNumber/1`
    );
  }
      ...
      <div className="col-3">
        ...
        <div className="filter-options-container">
          <div className="container-select-boxes-side-to-side">
            ...
            <div>
              <label htmlFor="averageCustomerRating">
              ...
              >
                <option value="" selected hidden>
                  {rating == 0 ? "Any" : rating + " or more stars"}
                </option>
                {ratings.map((r) => (
                  <>
          ...
          <div className="filter-search-button-container">
            ...
            >
              <i class="fa fa-search filter-search-icon"></i>Search
            </button>
          </div>
        </div>
        {loadingProducts ? (
            ...
            <div className="row center pagination">
              {[...Array(pages).keys()].map((x) => (
                <Link
                  ...
                  key={x + 1}
                  to={`/seller/${sellerId}/name/${name}/category/${category}/min/${min}/max/${max}/rating/${rating}/order/${order}/pageNumber/${
                    x + 1
                  }`}
                >
                  {x + 1}
