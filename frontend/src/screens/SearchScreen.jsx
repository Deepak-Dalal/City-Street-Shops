...
export default function SearchScreen() {
  ...
  function findMin(option) {
    ...
    else if (option == "₹1 to ₹100") return 1;
    else if (option == "₹101 to ₹1000") return 101;
    else return 1001;
  }
  function findMax(option) {
    if (option == "Any") return 0;
    else if (option == "₹1 to ₹100") return 100;
    else if (option == "₹101 to ₹1000") return 1000;
    else return 1000000;
  }
  useEffect(() => {
    dispatch(
        ...
        <div className="container-select-boxes-side-to-side">
          {loadingCategories ? (
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  navigate(
                    getFilterUrl({ category: e.target.value, pageNumber: 1 })
                  )
                }
              >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                navigate(
                  getFilterUrl({
                    min: findMin(e.target.value),
                    max: findMax(e.target.value),
                    pageNumber: 1,
                  })
                )
              }
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                navigate(
                  getFilterUrl({ rating: e.target.value, pageNumber: 1 })
                )
              }
            >
              {ratings.map((r) => (
          ...
          <div>
            ...
            <select
              ...
              value={order}
              onChange={(e) =>
                navigate(getFilterUrl({ order: e.target.value, pageNumber: 1 }))
              }
            >
          ...
          <div className="row center pagination">
            {[...Array(pages).keys()].map((x) => (
              <Link
                ...
                key={x + 1}
                to={`/search/name/${name}/category/${category}/min/${min}/max/${max}/rating/${rating}/order/${order}/pageNumber/${
                  x + 1
                }`}
              >
                {x + 1}
